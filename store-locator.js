...
(function () {
    window.Storelocator = {
        init: function (config) {
            // Get user location (GPS or IP fallback)
            ...
            // Delay init until location is ready
            getUserLocation((lat, lng) => {
                Storelocator.run(lat, lng, config);
            });
        },
        ...
        run: function (lat, lng, config) {
            ...
            const dataUrl = config.dataUrl ? "data/" + config.dataUrl : "data/stores.json"; // Default to "stores.json"
            const userLat = lat;
            const userLng = lng;
            ...
            fetch(dataUrl)
                ...
                .then(data => {
                    ...
                    // custom map function via lattitude and longitude
                    function getGoogleMapsLink(lat, lng) {
                        return `https://www.google.com/maps/embed?pb=!1m17!1m12!1m3!1d3682.6696056270466!2d${lng}!3d${lat}!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m2!1m1!2zMjLCsDM3JzQzLjciTiA4OMKwMjMnMzIuMCJF!5e0!3m2!1sen!2sin!4v1746640498139!5m2!1sen!2sin`;
                    }
                    ...
                    function renderStores(filteredStores) {
                        const storeItemsContainer = container.querySelector("#storeItems");
                        ...
                        filteredStores.forEach(store => {
                            store.map = store.map || getGoogleMapsLink(store.lat, store.lng);
                            store.address = store.address || `${store.city}, ${store.state} - ${store.postal_code}, ${store.country}`;
                        ...
                        // Create and append the map container dynamically if custom map is enabled
                        const mapContainer = container.querySelector('.sl-map-container');
                        if (config['custom-map'] && config['gmap-api']) {
                            mapContainer.innerHTML = "";
                            const mapDiv = document.createElement('div');
                            mapDiv.id = 'map';
                            mapDiv.classList.add('sl-map');
                            mapDiv.style.height = '600px'; // or any height you prefer
                            mapContainer.appendChild(mapDiv); // Add the map container to the DOM
                        }

                        // Set the first item as active by default
                        const storeItems = container.querySelectorAll(".sl-store-item");
                        if (storeItems.length > 0) {
                            storeItems[0].classList.add("active");
                            const firstMap = storeItems[0].getAttribute("data-map");
                            if (firstMap) {
                                updateMap(firstMap); // Use updateMap function to load map
                            }
                        ...
                        storeItems.forEach(item => {
                            item.addEventListener("click", function () {
                                const newMapSrc = this.getAttribute("data-map");
                                updateMap(newMapSrc); // Use updateMap function to load map
                        ...
                        callBtn.addEventListener("click", () => {
                            ...
                            const phoneEl = selectedStore.querySelector('.sl-store-phone');
                            if (phoneEl) {
                        ...
                        shareBtn.addEventListener("click", () => {
                            if (!selectedStore) return;
                            const map = selectedStore.getAttribute("data-map");
                            const lat = selectedStore.getAttribute("data-lat");
                            const lng = selectedStore.getAttribute("data-lng");
                            ...
                            if (lat && lng) {
                                ...
                                const match = map.match(/!3d([\d.]+)!2d([\d.]+)/);
                                if (match) {
                                    lat = match[1];
                                    lng = match[2];
                                }
                            ...
                            if (link) {
                                if (navigator.share) {
                                    navigator.share({
                                        ...
                                        text: "Check out this store location:",
                                        url: link
                                    }).catch(err => console.log("Share failed:", err));
                                } else {
                                    prompt("Copy this location URL:", link);
                                }
                            }
                        });
                        ...
                        dirBtn.addEventListener("click", () => {
                            if (!selectedStore) return;
                            let lat = selectedStore.getAttribute("data-lat");
                            let lng = selectedStore.getAttribute("data-lng");
                            const map = selectedStore.getAttribute("data-map");

                            // Try to extract from map URL if lat/lng missing
                            if ((!lat || !lng) && map) {
                                const match = map.match(/!3d([\d.]+)!2d([\d.]+)/);
                                if (match) {
                                    lat = match[1];
                    ...
                    // Function to update the map (either default or custom)
                    function updateMap(mapSrc) {
                        const mapIframe = container.querySelector(".sl-map-iframe");
                        const mapDiv = container.querySelector('#map');

                        // Check if custom map is requested
                        if (config['custom-map'] && config['gmap-api']) {
                            loadCustomMap(config['gmap-api'], mapSrc);
                        } else {
                            if (mapIframe) {
                                mapIframe.src = mapSrc; // Default map embed
                            }
                        }
                    }

                    // Function to load or reload the Google Maps script
                    function reloadGoogleMapsScript(apiKey) {
                        // Remove the existing Google Maps script if it exists
                    ...
                    // Function to load the custom Google Map with markers
                    function loadCustomMap(apiKey, mapSrc) {
                        // Load Google Maps JavaScript API dynamically
                        reloadGoogleMapsScript(apiKey);

                        // Define the callback function to initialize the map
                        window.initCustomMap = function () {
                            // Find the store item that has the "active" class
                            const activeItem = document.querySelector(".sl-store-item.active");

                            let initialLatLng;

                            // If an active item is found, get its `data-map` attribute and match the store
                            if (activeItem) {
                                const activeMapId = activeItem.getAttribute("data-map");
                                const matchingStore = data.find(store => store.map === activeMapId);
                                if (matchingStore) {
                                    initialLatLng = { lat: matchingStore.lat, lng: matchingStore.lng };
                                }
                            }

                            // Fallback to the first store if none is active or matched
                            if (!initialLatLng) {
                                initialLatLng = { lat: 22.563398361206055, lng: 88.35131072998047 };
                            }
                            ...
                            data.forEach(store => {
                                const storeItem = document.querySelector(`.sl-store-item[data-map="${store.map}"]`);
                                const icon = storeItem && storeItem.classList.contains('active') ? 'icons/highlighted-icon.png' : 'icons/default-icon.png';
                                ...
                                marker.addListener("click", function () {
                                    // Toggle the 'active' class on the corresponding store item
                                    storeItem.classList.toggle('active');

                                    // Update the marker icon based on the 'active' class
                                    marker.setIcon(storeItem.classList.contains('active') ? 'icons/highlighted-icon.png' : 'icons/default-icon.png');

                                    // Update the center to the clicked marker's position
                    ...
                    // Filter stores based on search input (by city)
                    ...
                    function performSearch() {
                        ...
                        const filteredStores = data.filter(store =>
                            ...
                            (store.pincode && store.pincode.toString().includes(query)) ||
                    ...
                    // Render all stores on initial load
                    renderStores(data);
                })
