 ...
 */
(function(){window.Storelocator={init:function(config){function getUserLocation(callback){if(navigator.geolocation){navigator.geolocation.getCurrentPosition((position)=>{callback(position.coords.latitude,position.coords.longitude)},()=>{getLocationFromIP(callback)})}else{getLocationFromIP(callback)}}
...
getUserLocation((lat,lng)=>{Storelocator.run(lat,lng,config)})},run:function(lat,lng,config){const container=document.querySelector(config.container);if(!container)return;const dataUrl=config.dataUrl?"data/"+config.dataUrl:"data/stores.json";const userLat=lat;const userLng=lng;const preloader=document.createElement('div');preloader.id='preloader';preloader.innerHTML=`
                <div class="preloader-inner">
                    <div class="spinner"></div>
                </div>
            `;container.appendChild(preloader);preloader.style.display='flex';fetch(dataUrl).then(res=>res.json()).then(data=>{preloader.style.display='none';container.innerHTML=`
...
function renderStores(filteredStores){const storeItemsContainer=container.querySelector("#storeItems");storeItemsContainer.innerHTML="";filteredStores.sort((a,b)=>{const distA=calculateDistance(userLat,userLng,a.lat,a.lng);const distB=calculateDistance(userLat,userLng,b.lat,b.lng);return distA-distB});filteredStores.forEach(store=>{store.map=store.map||getGoogleMapsLink(store.lat,store.lng);store.address=store.address||`${store.city}, ${store.state} - ${store.postal_code}, ${store.country}`;const distance=calculateDistance(userLat,userLng,store.lat,store.lng);const storeItem=`
                            ...
                            `;storeItemsContainer.innerHTML+=storeItem});const mapContainer=container.querySelector('.sl-map-container');if(config['custom-map']&&config['gmap-api']){mapContainer.innerHTML="";const mapDiv=document.createElement('div');mapDiv.id='map';mapDiv.classList.add('sl-map');mapDiv.style.height='600px';mapContainer.appendChild(mapDiv)}
const storeItems=container.querySelectorAll(".sl-store-item");if(storeItems.length>0){storeItems[0].classList.add("active");const firstMap=storeItems[0].getAttribute("data-map");if(firstMap){updateMap(firstMap)}
selectedStore=storeItems[0]}
storeItems.forEach(item=>{item.addEventListener("click",function(){const newMapSrc=this.getAttribute("data-map");updateMap(newMapSrc);storeItems.forEach(el=>el.classList.remove("active"));this.classList.add("active");selectedStore=this})});const callBtn=document.getElementById('call-btn');const shareBtn=document.getElementById('share-btn');const dirBtn=document.getElementById('direction-btn');callBtn.addEventListener("click",()=>{if(!selectedStore)return;const phoneEl=selectedStore.querySelector('.sl-store-phone');if(phoneEl){const phoneNumber=phoneEl.textContent
const number=phoneNumber.slice(-10);window.location.href=`tel:+91${number}`}});shareBtn.addEventListener("click",()=>{if(!selectedStore)return;const map=selectedStore.getAttribute("data-map");const lat=selectedStore.getAttribute("data-lat");const lng=selectedStore.getAttribute("data-lng");let link="";if(lat&&lng){link=`https://www.google.com/maps?q=${lat},${lng}`}else if(map){const match=map.match(/!3d([\d.]+)!2d([\d.]+)/);if(match){lat=match[1];lng=match[2]}
link=`https://www.google.com/maps?q=${lat},${lng}`}
if(link){if(navigator.share){navigator.share({title:"Store Location",text:"Check out this store location:",url:link}).catch(err=>console.log("Share failed:",err))}else{prompt("Copy this location URL:",link)}}});dirBtn.addEventListener("click",()=>{if(!selectedStore)return;let lat=selectedStore.getAttribute("data-lat");let lng=selectedStore.getAttribute("data-lng");const map=selectedStore.getAttribute("data-map");if((!lat||!lng)&&map){const match=map.match(/!3d([\d.]+)!2d([\d.]+)/);if(match){lat=match[1];lng=match[2]}}
if(lat&&lng){window.open(`https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`,"_blank")}});container.querySelector("#storeCount").textContent=filteredStores.length+" stores"}
...
function reloadGoogleMapsScript(apiKey){const existingScript=document.querySelector('script[src*="maps.googleapis.com/maps/api/js"]');if(existingScript){existingScript.remove()}
...
function loadCustomMap(apiKey,mapSrc){reloadGoogleMapsScript(apiKey);window.initCustomMap=function(){const activeItem=document.querySelector(".sl-store-item.active");let initialLatLng;if(activeItem){const activeMapId=activeItem.getAttribute("data-map");const matchingStore=data.find(store=>store.map===activeMapId);if(matchingStore){initialLatLng={lat:matchingStore.lat,lng:matchingStore.lng}}}
...
searchButton.addEventListener("click",performSearch);searchInput.addEventListener("keyup",function(event){if(event.key==="Enter"){performSearch()}});renderStores(data)}).catch(error=>console.error('Error fetching store data:',error))}}})()